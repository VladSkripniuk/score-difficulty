    ...
    for o in strm.flat.secondsMap:
        if o["element"].isClassOrSubclass(("Note",)):
            ...
            om_chord = [
                ...
                for oc in sorted(o["element"].notes, key=lambda a: a.pitch)
            ]
            om.extend(om_chord)
    om_filtered = []
    for o in om:
        ...
        simultaneous_notes = [
            o2
            for o2 in om
            if o2["offsetSeconds"] == offset and o2["element"].pitch.midi == pitch.midi
        ]
        max_duration = max([float(x["endTimeSeconds"]) for x in simultaneous_notes])
        if (
            len(simultaneous_notes) > 1
            and duration < max_duration
            and str(offset) + ":" + str(pitch) not in converted
        ):
            continue
        else:
        ...
        if (
            not (
                o["element"].tie
                and (
                    o["element"].tie.type == "continue"
                    or o["element"].tie.type == "stop"
                )
            )
            and not (
                (
                    hasattr(o["element"], "tie")
                    and o["element"].tie
                    and (
                        o["element"].tie.type == "continue"
                        or o["element"].tie.type == "stop"
            ...
            and not (o["element"].duration.quarterLength == 0)
        ):
